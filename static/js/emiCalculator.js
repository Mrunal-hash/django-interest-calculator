...
function calculateInterest(){
    ...
    if (isNaN(principal) || principal < 5000 || principal > 10000000) {
        document.getElementById("principalError").innerText = "Please enter a valid amount (Range between 5000-1000000).";
        isValid = false;
    }
    ...
    if (isNaN(time) || time < 1 || time > 1200) {
        document.getElementById("timeError").innerText = "Please enter a valid number of months.";
        isValid = false;
    }
    ...
    // Calculate the emi and interest
    rate = rate / (100*12);
    ...
    let emiInterest = totalAmount - principal;

    // display of the amounts on the screen
    document.getElementById("displayPrincipal").innerText = Intl.NumberFormat().format(emi.toFixed(2));
    document.getElementById("displayInterest").innerText = Intl.NumberFormat().format(emiInterest.toFixed(2));
    ...
    let ctx = document.getElementById("interestChart").getContext("2d");
            ...
            window.myChart = new Chart(ctx, {
                ...
                data: {
                    labels: ["Loan", "Interest"], // categories in the chart
                    datasets: [{
                        label: "Amount in ₹",
                        data: [principal, emiInterest], // their values
